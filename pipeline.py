...
def run_pipeline(wandb_project=WANDB_PROJECT, wandb_entity=WANDB_ENTITY):
    ...
    exclude_cols = ["month", "month_sin", "month_cos", "hour", "hour_sin", "hour_cos", "day_of_week", "day_of_week_sin", "day_of_week_cos"]
    ref_eng_filtered   = ref_eng.drop(columns=[c for c in ref_eng.columns if c in exclude_cols or c.startswith("PULocationID_") or c == "PULocationID"])
    current_2026_eng_filtered = current_2026_eng.drop(columns=[c for c in current_2026_eng.columns if c in exclude_cols or c.startswith("PULocationID_") or c == "PULocationID"])
    ...
    PROTECTED = ["hour", "month", "day_of_week", "hour_sin", "hour_cos", "day_of_week_sin", "day_of_week_cos", "month_sin", "month_cos"]
    ...
    if selected_strategy == "none":
        ...
        mitigated_model, mitigated_scaler, mitigated_mappings, dropped_features = mitigate(
        ...
        if dropped_features:
            current_2026_eng_mit = current_2026_eng_mit.drop(columns=dropped_features, errors='ignore')
