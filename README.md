...
For the pipeline to run successfully, you need to configure your Weights & Biases (W&B) credentials:
1. Create a free account on [Weights & Biases](https://wandb.ai/).
2. Retrieve your API key from your W&B account settings.
3. Create a file named `wandb_key.txt` in the root directory of the repository and paste your API key inside.

> [!NOTE]
> Without the `wandb_key.txt` file, the pipeline cannot log metrics and will not run.
...
| **10** | **Mitigation** | Applies automated mitigation strategies based on detected drift type. |
...
```
> [!IMPORTANT]
> To enable experiment tracking, create a `wandb_key.txt` file in the repository root and paste your Weights & Biases API key into it. The pipeline will not run without this file.


### Run via GitHub Actions
A CI/CD workflow is provided in `.github/workflows/pipeline.yml`. It is automatically triggered on pushes to the `main` branch that modify the source code (`src/`), orchestration script (`pipeline.py`), dependencies (`requirements.txt`), or raw data (`data/raw/`). It can also be executed manually through the GitHub Actions interface.
