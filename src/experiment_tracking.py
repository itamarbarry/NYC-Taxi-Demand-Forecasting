...
class ExperimentTracker:
    ...
    def log_artifact(self, file_path, artifact_name: str,
        ...
        """
        print(f"  W&B: Uploading {artifact_type} artifact '{artifact_name}' ({Path(file_path).name}) ...")
        ...
        print(f"  W&B: Artifact '{artifact_name}' versioned successfully.")
    ...
    def log_code(self, root: str = "."):
        ...
        def _is_code_file(path):
            # 1. Skip large/non-source directories early
            # 'path' is relative to root
            parts = path.split(os.sep)
            for skip in [".venv", "data", "models", "outputs", ".git", "__pycache__", ".ipynb_checkpoints"]:
                if skip in parts:
                    return False
            
            # 2. Only include python files and notebooks
            return path.endswith(".py") or path.endswith(".ipynb")

        self.run.log_code(
            root       = root,
            include_fn = _is_code_file
        )
    ...
    def finish(self) -> str:
        ...
        url = self.run.url
        print(f"  W&B: Finishing run and syncing final metadata ...")
        self.run.finish()
        return url
