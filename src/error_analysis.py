...
def build_error_df(y_test, y_pred, feature_df=None, scaler=None, mappings=None):
    """
    ...
    # ── Reconstruct one-hot encoded columns & target-encoded columns ─────────
    borough_cols = [c for c in feat.columns if c.startswith("Borough_")]
    if borough_cols:
        feat["Borough"] = feat[borough_cols].idxmax(axis=1).str.replace("Borough_", "", regex=False)
        
    service_zone_cols = [c for c in feat.columns if c.startswith("service_zone_")]
    if service_zone_cols:
        feat["service_zone"] = feat[service_zone_cols].idxmax(axis=1).str.replace("service_zone_", "", regex=False)
        
    if "PULocationID" in feat.columns:
        if mappings is not None and "location_reverse_enc" in mappings:
            feat["PULocationID"] = feat["PULocationID"].map(mappings["location_reverse_enc"]).astype(float)
        ...
        try:
            ...
            if scaled_cols:
                ...
                for col in ["hour", "day_of_week", "month", "PULocationID", "Borough", "service_zone"]:
                    if col in feat.columns and pd.api.types.is_numeric_dtype(feat[col]):
                        if col == "hour":
                            # Keep fractional hour precision (e.g. 16.5)
    ...
    if "Borough" in feat.columns:
        if pd.api.types.is_numeric_dtype(feat["Borough"]):
            error_df["Borough"] = feat["Borough"].map(BOROUGH_NAMES).fillna("Unknown")
        else:
            error_df["Borough"] = feat["Borough"].fillna("Unknown")

    # ── Service Zone ──────────────────────────────────────────────────────────
    if "service_zone" in feat.columns:
        if pd.api.types.is_numeric_dtype(feat["service_zone"]):
            error_df["service_zone"] = feat["service_zone"].map(SERVICE_ZONE_NAMES).fillna("Unknown")
        else:
            error_df["service_zone"] = feat["service_zone"].fillna("Unknown")
    ...
    if "hour" in feat.columns:
        non_null_hours = feat["hour"].dropna()
        if scaler is not None or (len(non_null_hours) > 0 and non_null_hours.min() >= 0.0 and non_null_hours.max() <= 24.5):
            def get_hour_bin(h):
                h = h % 24
                if 6 <= h < 12:
            ...
            error_df["hour"] = feat["hour"].apply(get_hour_bin)
        else:
...
def plot_error_histogram_by_segment(error_df, group_col, title=None, output_dir=None, bins=40):
    ...
    # Custom chronological sorting for hour/time of day bins
    def get_sort_key(grp):
        grp_str = str(grp)
    ...
    for i, grp in enumerate(sorted(groups, key=get_sort_key)):
...
def run_error_analysis(X_test, y_test, model, scaler=None, mappings=None, output_dir=None):
    """
    Full error analysis pipeline for demand.
    """
    y_pred   = model.predict(X_test)
    error_df = build_error_df(y_test, y_pred, feature_df=X_test, scaler=scaler, mappings=mappings)
