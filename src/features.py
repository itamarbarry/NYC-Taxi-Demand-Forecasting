...
# Numerical features to scale (excluding PULocationID which is now one-hot encoded)
SCALE_FEATURES = ["hour", "day_of_week", "month"]
...
def _add_cyclical_time_features(df):
    """
    Add sin/cos transformations for hour, day of week, and month.
    ...
    # day_of_week ranges from 1 to 7 (1=Sun, ..., 7=Sat)
...
def run_feature_pipeline(df, scaler=None, mappings=None, is_training=True,
    ...
    # 2. Categorical Encoding (OHE for Borough/service_zone, Target Encoding for PULocationID)
    ohe_cols = ["Borough", "service_zone"]
    
    if is_training:
        from sklearn.preprocessing import OneHotEncoder
        mappings = {}
        
        # A. Fit One-Hot Encoder for Borough and service_zone
        encoder = OneHotEncoder(sparse_output=False, handle_unknown="ignore")
        encoder.fit(df[ohe_cols])
        mappings["encoder"] = encoder
        
        # B. Calculate Target Encoding for PULocationID
        location_map = df.groupby("PULocationID")["demand"].mean().to_dict()
        global_mean = float(df["demand"].mean())
        mappings["location_target_enc"] = location_map
        mappings["global_mean"] = global_mean
        
        # Reverse mapping for reconstruction in error analysis / plotting
        mappings["location_reverse_enc"] = {v: k for k, v in location_map.items()}
    else:
        encoder = mappings.get("encoder")
        location_map = mappings.get("location_target_enc")
        global_mean = mappings.get("global_mean")
        if encoder is None or location_map is None or global_mean is None:
            raise ValueError("Required encoders/maps not found in mappings when is_training=False.")

    # Apply One-Hot Encoding to Borough and service_zone
    encoded_cats = encoder.transform(df[ohe_cols])
    ...
    # Apply Target Encoding to PULocationID
    df["PULocationID"] = df["PULocationID"].map(location_map).fillna(global_mean)
