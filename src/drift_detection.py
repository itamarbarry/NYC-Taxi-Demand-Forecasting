...
def run_monthly_drift_analysis(monthly_eval_df, reference_raw_df,
    ...
    if len(monthly_summary) and output_dir:
        ...
        # Reconstruct PULocationID for distribution plotting if it was one-hot encoded or target-encoded
        for eng_df in [ref_eng, worst_eng]:
            pu_cols = [c for c in eng_df.columns if c.startswith("PULocationID_")]
            if pu_cols:
                eng_df["PULocationID"] = eng_df[pu_cols].idxmax(axis=1).str.replace("PULocationID_", "", regex=False).astype(float)
            elif "PULocationID" in eng_df.columns and mappings is not None and "location_reverse_enc" in mappings:
                eng_df["PULocationID"] = eng_df["PULocationID"].map(mappings["location_reverse_enc"]).astype(float)
