...
DRIFT_THRESHOLDS = {
    ...
    # Minimum number of severely drifted features needed to trigger
    # the "drop_features" mitigation strategy.
...
def select_mitigation_strategy(drift_results: dict,
    ...
    Decision logic:
      We are treating the 2026 data as a true "locked" test set,
      therefore we are FORBIDDEN from using 2026 data to change ANY model or scaler parameters.
      This means that strategies modifying parameters using 2026 data (like recalibrating the scaler) are illegal.
      
      Our only allowed mitigation is "drop_features" because it removes the bad
      columns and retrains entirely on the OLD 2024-2025 training data.
      
      1. If there are severely drifted features:
             -> "drop_features"  (Drop them and retrain on 2024-2025 data)
      2. Otherwise:
             -> "none"           (We cannot mitigate because doing so would require 2026 data)
    ...
    # Only if we actually have features we can drop
    ...
    # If there's concept drift but no specific features to drop, we're stuck.
    if concept_drift_results and concept_drift_results["concept_drift_detected"]:
        pct = concept_drift_results["mae_pct_increase"]
        print(f"  Concept drift detected: MAE increased by {pct:.1%}.")
        print("  However, no severe features identified to drop. Because updating scaler/model parameters using 2026 data is forbidden, strategy: none")
        return "none"

    # If there's overall drift but no specific features to drop, we're also stuck.
    if drift_results["overall_drift"]:
        share = drift_results["share_drifted"]
        print(f"  Widespread dataset drift detected ({share:.1%} of features drifted).")
        print("  However, no individual severe features identified to drop. Because updating parameters using 2026 data is forbidden, strategy: none")
        return "none"
